...
import com.teaching.common.core.domain.entity.SysStudent;
...
import com.teaching.system.service.ISysDeptService;
...
public class SysUserController extends BaseController
{
    ...
    @Log(title = "用户管理-学生", businessType = BusinessType.IMPORT)
    @PreAuthorize("@ss.hasPermi('system:user:import')")
    @PostMapping("/importStudentData")
    public AjaxResult importStudentData(MultipartFile file, boolean updateSupport) throws Exception
    {
        ExcelUtil<SysStudent> util = new ExcelUtil<SysStudent>(SysStudent.class);
        List<SysStudent> studentList = util.importExcel(file.getInputStream());
        List<SysDept> depts = deptService.selectDeptList(new SysDept());
        List<SysRole> roles = roleService.selectRoleAll();
        Long roleId = roles.stream().filter(a -> a.getRoleKey().equals("student")).findFirst().get().getRoleId();
        ...
        for (SysStudent sysStudent : studentList) {
            if (deptMap.containsKey(sysStudent.getDeptId())) {
                SysUser sysUser = new SysUser();
                sysUser.setUserName(sysStudent.getUserName());
